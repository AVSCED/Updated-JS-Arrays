    ...
    for (i = 0; i < disp_arr.length; i++) {
      if (disp_arr[i].product_id == stock) {
        if (quantity <= disp_arr[i].product_quantity) {
          value = stock;
          disp_arr[i].product_quantity =
            disp_arr[i].product_quantity - quantity;
...
function generateBill() {
  ...
  for (i = 0; i < disp_arr.length; i++) {
    s =
      " <tr><th>Company :</th> <th>Model :</th>  <th>Quantity :</th>  <th>Price :</th></tr> ";

    if (disp_arr[i].flag == 1) {
      var bill = 1;
      var cartItem = 0;
      value = key_value;
      console.log(typeof disp_arr[0].product_price);
      bill = Number(disp_arr[i].product_price);
      cartItem = 20 - disp_arr[i].product_quantity;
      ...
      s = disp_arr.forEach((value) => {
        if (value.flag == key_value) {
          ...
          s +=
            ...
            bill +
            "</td><td>";
          document.getElementById("table4").innerHTML = s;
